...
import { rewards, maxPointsNewProduct, maxPointsNewWatchBonusFull } from "src/constants/gamification";
...
const RewardsBanner = () => {
  ...
  useEffect(() => {
    const R = {
      ...
      productionYearsR: values?.productionYearStart && values?.productionYearEnd ? rewards.PRODUCT_YEARS : 0,
      productCaseSizeR: values?.caseSize ? rewards.PRODUCT_CASE_SIZE : 0,
      productCaseMaterialR: values?.caseMaterial ? rewards.PRODUCT_CASE_MATERIAL : 0,
      productWaterResR: values?.waterResistance ? rewards.PRODUCT_WATER_RESISTANCE : 0,
      productPriceR: values?.productPriceBrackets ? rewards.PRODUCT_PRICE : 0,
      productPicturesR: values?.productThumbnail.length > 0 ? rewards.PRODUCT_PICTURE * values?.productThumbnail.length : 0,
      productDescriptionR: values?.productDesc ? rewards.PRODUCT_DESCRIPTION : 0,
      productAdditionalDataR: values?.additionalData.length > 0 ? rewards.PRODUCT_ADDITIONAL_DATA * values?.additionalData.length : 0
    }
    const sum = sumValuesObject(R)
    setReward(sum < maxPointsNewProduct ? sum : sum + maxPointsNewWatchBonusFull)
